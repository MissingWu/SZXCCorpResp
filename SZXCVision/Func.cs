...
namespace VisionCore
{
    ...
    public class Func
    {
        /// <summary>服务器socket给客户端发送信息</summary>
        public static void SendMsg(ECom com, string str)
        {
        ...
        public static void MeasLine(HImage image, ROILine line, Meas_Info meas, out ROILine outLine, out HTuple outR, out HTuple outC, out HXLDCont outXld, HRegion disableRegion = null)
        {
            ...
            try
            {
                ...
                HTuple lineInfo = (new HTuple(new double[] { line.StartX, line.StartY, line.EndX, line.EndY }));
                //最强边的计算
                ...
                {
                    outLine = new ROILine();
                    outLine.StartX = Math.Round(lineResult[0].D, 4);
                    outLine.StartY = Math.Round(lineResult[1].D, 4);
                    outLine.EndX = Math.Round(lineResult[2].D, 4);
                    outLine.EndY = Math.Round(lineResult[3].D, 4);
        ...
        public static void MeasCircle(HImage image, ROICircle circel, Meas_Info meas, HRegion disableRegion, out ROICircle outCircle, out HTuple outR, out HTuple outC, out HXLDCont outXld)
        {
            ...
            try
            {
                ...
                Circle_Info.Append(new HTuple(new double[] { circel.CenterX, circel.CenterY, circel.Radius }));
                MetroModel.AddMetrologyObjectGeneric
                    (new HTuple("circle"),
                    Circle_Info,
                    new HTuple(meas.Length1),
        ...
        public static void MeasPairs(HImage image, Rect2_Info inRectangle2, Meas_Info meas, out List<ROILine> lstLine, out List<double> lstWidth, out List<double> lstDistance, out HXLDCont outXld)
        {
        ...
        public static void RectPosition(RImage img, List<Rect2_Info> inRectangleList, Coord_Info inCoordInfo, out List<Rect2_Info> outRectangleList)
        {
            try
            {
                ...
                foreach (Rect2_Info r in inRectangleList)
                {
                    ...
                    Func.WorldPlane2Point(img, x, y, out row, out col);
        ...
        public static void CreateModel(ModelType mType, HImage image, RexView.ROI SearchRegion, RexView.ROI TempRegion, int Threshold, ref HHandle TempHandle, double StartPhi, double EndPhi)
        {
            try
            {
                HRegion _region = TempRegion.GetRegion();
        ...
        public static int FindModel(ModelType _Type, HHandle _Model, double StartPhi, double EndPhi, double _MinScore, HImage image, RexView.ROI _roi, out Coord_Info outCoord)
        {
            int num = 0;
            ...
            try
            {
                ...
                {
                    HRegion _region = _roi.GetRegion();
                    if (_region.IsInitialized())
                    {
                        image = image.ReduceDomain(_region);
                    }
                    if (_Type == ModelType.形状模板)
                    {
                        ...
                        if (score.Length > 0)
                        {
                            outCoord.X = Math.Round(row[0].D, 4);
                            outCoord.Y = Math.Round(col[0].D, 4);
        ...
        public static Rect2_Info Rectangle2WorldPlane(RImage img, Rect2_Info inRectangle2)
        {
            Rect2_Info outRectangle2 = new Rect2_Info();
            try
            {
                Points2WorldPlane(img, inRectangle2.CenterY, inRectangle2.CenterX, out inRectangle2.CenterX, out inRectangle2.CenterY);
        ...
        public static HXLDCont GetCoord(RImage img, Coord_Info coord)
        {
            ...
            Func.GenArrow(out HXLDCont CoordXLD, row1, col1, row2, col2, 5, 5);
            Func.WorldPlane2Point(img, coord.Y, coord.X, out double row, out double col);
        ...
        public static HXLDCont GetCoordShort(RImage img, Coord_Info coord)
        {
            ...
            Func.GenArrow(out HXLDCont CoordXLD, row1, col1, row2 / 30, col2 / 20, 4, 4);
            Func.WorldPlane2Point(img, coord.X, coord.Y, out row, out col);
            HHomMat2D hom = new HHomMat2D();
            hom.VectorAngleToRigid(0, 0, 0, row, col, coord.Phi);
    ...
    public class Var
    {
        ...
        {
            DataCell mDataCell = inVar.FirstOrDefault(c => c.mModName == Name);
            object image = mDataCell.mDataValue;
            if (image == null) { ShowMsg.SendLog(LogLevel.Error, Name + ": 查找图像失败！"); }
            return image;
        }
        /// <summary>
        /// 通过单元ID和名称查找变量列表
        ...
        /// <returns></returns>
        public static string GetLinkData(List<DataCell> VarList, string Name, string DataName)
        {
            string OutData = Name + "|" + DataName + "|";
            try
            {
        ...
        public static Dictionary<string, object> GetFields<T>(T t)
        {
            ...
            foreach (FieldInfo item in fields)
            {
                ...
                if (item.FieldType.IsValueType || item.FieldType.Name.StartsWith("String") || item.FieldType.Name.StartsWith("Boolean"))
                {
    ...
    public class Fit
    {
        ...
        public static bool FitLine(List<double> rows, List<double> cols, out ROILine line)
        {
            ...
            try
            {
                Func.SortPairs(ref rows, ref cols);
